...
data_train_feature_censored=list()
for (i in 1:500){
  data_train<-data_sim_train[[i]]
  data_train<-subset(data_train,CVD_DTH_UNADJ_sim==0)
...
data_train_feature_censored[[i]]<-cbind(data_pst,V1=V1,V2=V2,V3=V3)
}
save(data_train_feature_censored,file="data_train_feature_censored.RData")
