...
data_test_feature_censored=list()
for (i in 1:500){
  data_test<-data_sim_test[[i]]
  data_test<-subset(data_test,CVD_DTH_UNADJ_sim==0)
  data_test$new_id <- match(data_test$id, unique(data_test$id))
  data_test.id<-data_test[!duplicated(data_test$new_id), ]# to get baseline info
  table(data_test.id$CVD_DTH_ADJ_sim)
  
  data=data_test
  data.id=data_test.id
  T=15 #maximum time
  legP2<-function(t){
    #T<-max(t)
...
data_test_feature_censored[[i]]<-cbind(data_pst,V1=V1,V2=V2,V3=V3)
}
save(data_test_feature_censored,file="data_test_feature_censored.RData")
